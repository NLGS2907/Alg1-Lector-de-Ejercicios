...
class CogMisc(CogGeneral):
    ...
    @command(name="info", aliases=["i"], usage="-dm", help="Muestra una lista de todos los comandos.")
    async def mostrar_info(self, ctx: Context, *opciones):
        ...
        embed = Embebido(version_bot=version_bot,
        ...
        if "-dm" in opciones:
            ...
            await mandar_dm(ctx, contenido=f"{ctx.author.mention} ha consultado:", view=info_ui, embed=embed)

        else:

            await ctx.channel.send(content=f"{ctx.author.mention} ha consultado:", view=info_ui, embed=embed)


    @command(name="version", aliases=["ver"], help="Muestra la versión del bot.")
    ...
    @command(name="whatsnew", aliases=["quehaydenuevo", "nuevo"], help="Muestra las novedades de la versión actual.", hidden=True)
